...
% if not available assigned vh=0.0;
vh=0.0; %m/s
...
if requirement == "-CSVLA"
    ...
    para = Paragraph(strcat('Check regulations. If different from CS-VLA to be updates HERE'));
    para.Style = {HAlign('justify')};
    add(sec,para)
end
...
elseif(rpt.Type == "docx")
    ...
    para = Paragraph(strcat('where M/S is the wing loading in kg/m2 and g is the acceleration due to gravity in m/s2.'));
    add(sec,para)                        
    %VCmin    
...
if requirement == "-CSVLA"
    ...
    if vcmin <= 1.25*Aircraft.Certification.Regulation.SubpartC.Flightloads.Final_envelope.PointC.VC.value
        para = Paragraph(strcat('VCmin not available. Other subsection will be used',...
            ' VD = 1.25VC = ',...
        ...
        para = Paragraph(strcat('- VD = 1.25VC = ',...
            num2str(1.25*Aircraft.Certification.Regulation.SubpartC.Flightloads.Final_envelope.PointC.VC.value),...
                                'VD = 1.4VCmin = ',...
            num2str(1.4*vcmin)));        
        add(sec,para)
...
if requirement == "-CSVLA"
    para = Paragraph('VNE is not a design airspeeds. It must be checked into sec. CS-VLA 1505 Airspeed limitations.');
    para.Style = {HAlign('justify')};
    add(sec,para)
else
    para = Paragraph(strcat('Check regulations. If different from CS-VLA to be updates HERE'));
