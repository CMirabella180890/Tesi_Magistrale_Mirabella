...
% Comment this line to store data
...
% ac.name = 'PROSIB_WT';                              % aircraft name
...
ac.wing.twist       = [3.0 3.0];               % twist angles
...
ac.wing.span        = [Aircraft.Geometry.Wing.b.value*0.5];     % span(s)
ac.wing.ctip        = [Aircraft.Geometry.Wing.ctip.value];  % tip chord(s)
ac.wing.croot       = [Aircraft.Geometry.Wing.croot.value];  % root chord(s)
...
% Aircraft.Certification.Regulation.SubpartC.Balancingloads.OpenVSPDiagram.value = PlotResults(set,res,ac.name,ac.config);
