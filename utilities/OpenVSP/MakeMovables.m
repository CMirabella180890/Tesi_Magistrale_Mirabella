...
eta_inner = wing.mov.eta_inner;
eta_outer = wing.mov.eta_outer;
cf_c_inner = wing.mov.cf_c_inner;
cf_c_outer = wing.mov.cf_c_outer;
tessellation = wing.mov.tessellation;
...
fprintf(fileid,'%s\r\n',['SetParmVal( ' wing.id ' , "UStart", "SS_Control_1", ' num2str(eta_vsp_inner) ');']); 
fprintf(fileid,'%s\r\n',['SetParmVal( ' wing.id ' , "UEnd", "SS_Control_1", ' num2str(eta_vsp_outer) ');']); 
...
fprintf(fileid,'%s\r\n',['SetParmVal( ' wing.id ' , "Length_C_Start", "SS_Control_1", ' num2str(cf_c_inner) ');']); 

% tasselation on edges 
fprintf(fileid,'%s\r\n',['SetParmVal( ' wing.id ' , "Tess_Num", "SS_Control_1", ' num2str(tessellation) ');']); 
