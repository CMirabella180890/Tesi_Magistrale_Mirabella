...
prop_polar_mom        = Aircraft.Engine.Propeller_polar_moment.value;
normal_load_factor    = Aircraft.Engine.Engine_normal_load_factor.value;
...
% PROPELLER ROTATIONAL SPEED AT TAKEOFF POWER
prop_rotational_speed_takeoff = (takeoff_rpm) / (reduction_ratio);
Aircraft.Engine.Takeoff.Propeller_rotational_speed.value = prop_rotational_speed_takeoff;
Aircraft.Engine.Takeoff.Propeller_rotational_speed.Attributes.unit = "RPM";

% PROPELLER ROTATIONAL SPEED AT MAX CONTINOUS POWER
prop_rotational_speed_maxcontinous = (max_continous_rpm) / (reduction_ratio);
Aircraft.Engine.Max_Continous.Propeller_rotational_speed.value = prop_rotational_speed_maxcontinous ;
Aircraft.Engine.Max_Continous.Propeller_rotational_speed.Attributes.unit = "RPM";
...
% PROPELLER'S NUMBER OF BLADES 
number_of_blades = Aircraft.Engine.Propeller_blade_number.value;

% A WAY TO DECIDE HOW TO EVALUATE GYROSCOPIC LOADS 
if number_of_blades == 2
    Gyro_loads = @(I_p, omega_1, omega_2) 2 * I_p * omega_1 * omega_2; 
elseif number_of_blades >= 3 
    Gyro_loads = @(I_p, omega_1, omega_2) I_p * omega_1 * omega_2; 
end

Applicable_inertia_load = normal_load_factor * Engine_block_weight;
